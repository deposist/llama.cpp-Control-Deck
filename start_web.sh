...
usage() {
  cat <<'EOF'
Usage: start_web.sh [--setup] [web options]

Starts the llama.cpp Control Deck web panel, then open the printed URL
in a browser. Every service is started and stopped from that page.

Beginner path:
  ./start_web.sh --setup   Create/update .venv, install requirements, then start.
...
All options except -h/--help and --setup are passed to control_web.py.
EOF
}
...
# The web panel needs no Tkinter, only the HTTP dependencies.
...
cd "$SCRIPT_DIR"
exec "$PYTHON" "$SCRIPT_DIR/control_web.py" "${WEB_ARGS[@]}"
